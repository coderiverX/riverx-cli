...
- [x] 最大 tool-use 轮次限制（默认 200 轮）
- [x] 每轮工具执行前检查是否需要 confirm
...
- [x] 启动 REPL → 多轮对话 → 上下文正确保持
- [x] REPL 中执行高危命令 → 确认流程正常
- [x] `/clear` 后上下文重置
- [x] 退出后会话保存到 sessions/
- [x] 日志正常写入
...
- [x] LLM API 网络错误 → 友好提示 + 重试建议
- [x] LLM API key 无效 → 明确提示如何配置
- [x] LLM API 限流 → 提示等待
- [x] LLM 返回格式异常 → 降级为纯文本输出
- [x] 工具执行异常 → 错误信息返回给 LLM 让其决策
- [x] 文件权限不足 → 明确提示
- [x] 磁盘空间不足 → 写文件前检查
...
- [x] LLM 文本响应支持 Markdown 渲染（终端基础格式：粗体、代码块、列表）
- [x] 工具执行进度动画（spinner）
- [x] 命令输出语法高亮（可选）
- [x] 错误信息用红色显示
- [x] 成功信息用绿色显示
